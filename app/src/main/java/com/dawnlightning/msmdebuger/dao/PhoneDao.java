...
public class PhoneDao implements PhoneDaoInface {
	...
	public boolean addCache(Phone item) {
		...
		try {
			...
			values.put(SQLHelper.PHONE, item.getNumber());
			values.put(SQLHelper.ADDRESS, item.getAddress());
			values.put(SQLHelper.CODE, item.getCode());
	...
	public boolean addAdmin(Contact item) {
		...
		try {
			...
			ContentValues values = new ContentValues();
			values.put(SQLHelper.PHONE, item.getNumber());
			values.put(SQLHelper.REMARK, item.getName());
			id = database.insert(SQLHelper.TABLE_ADMIN, null, values);
			flag = (id != -1 ? true : false);
		} catch (Exception e) {
			// TODO: handle exception
		} finally {
	...
	public boolean deleteAdmin(String whereClause, String[] whereArgs) {
		...
		try {
			database = helper.getWritableDatabase();
			count = database.delete(SQLHelper.TABLE_ADMIN, whereClause, whereArgs);
			flag = (count > 0 ? true : false);
		} catch (Exception e) {
			// TODO: handle exception
		} finally {
	...
	@Override
	public boolean updateAdmin(ContentValues values, String whereClause, String[] whereArgs) {
		// TODO Auto-generated method stub
		boolean flag = false;
		SQLiteDatabase database = null;
		int count = 0;
		try {
			database = helper.getWritableDatabase();
			count = database.update(SQLHelper.TABLE_ADMIN, values, whereClause, whereArgs);
			flag = (count > 0 ? true : false);
		} catch (Exception e) {
			// TODO: handle exception
		} finally {
	...
	@Override
	public List<Map<String, String>> listAdmin(String selection, String[] selectionArgs) {
		// TODO Auto-generated method stub
		List<Map<String, String>> list = new ArrayList<Map<String, String>>();
		SQLiteDatabase database = null;
		Cursor cursor = null;
		try {
			database = helper.getReadableDatabase();
			cursor = database.query(false, SQLHelper.TABLE_ADMIN, null, selection,selectionArgs, null, null, null, null);
			int cols_len = cursor.getColumnCount();
			while (cursor.moveToNext()) {
				Map<String, String> map = new HashMap<String, String>();
				for (int i = 0; i < cols_len; i++) {
